    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                <h1 className="text-2xl font-bold text-gray-900">Wiadomości Kontaktowe</h1>
            ...
            <div className="bg-white shadow rounded-xl border border-gray-200 overflow-hidden">
                {messages.length > 0 ? (
                    <ul className="divide-y divide-gray-100">
                        {messages.map((msg) => (
                            <li key={msg.id} className={`p-6 hover:bg-gray-50 transition ${msg.status === "NEW" ? "bg-indigo-50/30" : ""}`}>
                                <div className="flex flex-col md:flex-row gap-4 justify-between">
                                    <div className="flex-1 space-y-2">
                                        <div className="flex items-center gap-3">
                                            <span className="text-xs font-mono font-bold text-gray-400">#{msg.number}</span>
                                            <MessageStatusSelect id={msg.id} currentStatus={msg.status} />
                                            <h3 className="font-semibold text-gray-900 ml-2">{msg.subject || "(Bez tematu)"}</h3>
                                        </div>
                                        ...
                                        <p className="text-gray-700 whitespace-pre-wrap text-sm leading-relaxed max-w-3xl border-l-2 border-indigo-100 pl-3 py-1">
                                            {msg.message}
                                        </p>
