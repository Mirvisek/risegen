    ...
    <div className="flex flex-col gap-16 pb-16 bg-white">
      ...
      {showNews && (
        <section className="container mx-auto px-4 max-w-6xl space-y-12 animate-in fade-in slide-in-from-bottom-10 duration-700">
          <div className="text-center space-y-4">
            <h2 className="text-3xl font-bold text-gray-900">Aktualności</h2>
            <div className="w-16 h-1 bg-indigo-600 mx-auto rounded-full"></div>
            <p className="text-gray-600">Bądź na bieżąco z życiem naszego stowarzyszenia.</p>
          </div>
          ...
          {latestNews.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {latestNews.map((news) => {
                ...
                return (
                  <div key={news.id} className="group bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-100 flex flex-col h-full transform hover:-translate-y-1">
                    ...
                    <div className="p-6 flex-grow flex flex-col">
                      <div className="text-xs text-indigo-600 font-bold mb-3 bg-indigo-50 inline-block px-3 py-1 rounded-full w-fit">
                        {new Date(news.createdAt).toLocaleDateString("pl-PL")}
                      </div>
                      <h3 className="text-xl font-bold text-gray-900 mb-3 group-hover:text-indigo-600 transition-colors">{news.title}</h3>
                      <p className="text-gray-600 line-clamp-3 mb-6 flex-grow text-sm leading-relaxed">
                        {news.content.replace(/<[^>]*>?/gm, '').substring(0, 150)}...
                      </p>
                      <Link href={`/aktualnosci/${news.slug}`} className="text-indigo-600 font-bold hover:text-indigo-800 mt-auto inline-flex items-center group/link transition-colors">
            ...
            <div className="text-center py-16 bg-gray-50 rounded-2xl border border-dashed border-gray-300">
              <p className="text-gray-500">Brak aktualności. Wkrótce coś dodamy!</p>
            </div>
          )}
        </section>
      )}
      ...
      {showProjects && (
        <section className="container mx-auto px-4 max-w-6xl space-y-12 animate-in fade-in slide-in-from-bottom-10 duration-700 delay-100">
          <div className="text-center space-y-4">
            <h2 className="text-3xl font-bold text-gray-900">Nasze Projekty</h2>
            <div className="w-16 h-1 bg-indigo-600 mx-auto rounded-full"></div>
            <p className="text-gray-600">Działamy aktywnie, realizując inicjatywy ważne dla regionu.</p>
          </div>
          ...
          {featuredProjects.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {featuredProjects.map((project) => {
                ...
                return (
                  <div key={project.id} className="group bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-100 flex flex-col h-full transform hover:-translate-y-1">
                    <div className="h-56 bg-gray-200 w-full relative">
                      {imageUrl ? (
                        <img
                          src={imageUrl}
                        ...
                        <div className="w-full h-full flex items-center justify-center text-gray-400 font-medium">Projekt RiseGen</div>
                      )}
                    </div>
                    <div className="p-6 flex-grow flex flex-col">
                      <h3 className="text-xl font-bold text-gray-900 mb-3 group-hover:text-indigo-600 transition-colors">{project.title}</h3>
                      <p className="text-gray-600 line-clamp-3 mb-6 flex-grow leading-relaxed font-light">{project.description ? project.description.replace(/<[^>]*>?/gm, '').substring(0, 150) + "..." : ''}</p>
                      <Link href={`/projekty/${project.slug}`} className="text-indigo-600 font-bold hover:text-indigo-800 mt-auto inline-flex items-center group/link transition-colors">
            ...
            <div className="text-center py-16 bg-gray-50 rounded-2xl border border-dashed border-gray-300">
              <p className="text-gray-500">Na razie nie wyróżniliśmy żadnych projektów. Wkrótce się pojawią!</p>
            </div>
          )}
        </section>
      )}
